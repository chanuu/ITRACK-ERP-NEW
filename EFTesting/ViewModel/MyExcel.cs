...
namespace MyTeamApp
{
    class ExcelHelpers
    {
       ...
       private List<OperationPool> GetoprationByID(string ID)
       {
        ...
        public static List<SerialEntry> fabList = new List<SerialEntry>();
        public List<SerialEntry> ReadFabricDetails(DevExpress.XtraEditors.ProgressBarControl bar)
        {
            ...
            for (int index = 2; index <= lastRow; index++)
            {
                ...
                string Color = "";
                bool _hasData = false;
                SerialEntry record = new SerialEntry();

                if (MySheet.Cells[index, 1].Value != null)
                {
                    SRNO = Convert.ToInt32((string)(MySheet.Cells[index, 1].Value.ToString()));
                    record.SRNo = SRNO;
                ...
                {
                    RollNo = (string)(MySheet.Cells[index, 2].Value.ToString());
                ...
                {
                    TotalLength = Convert.ToDouble((string)(MySheet.Cells[index, 3].Value.ToString()));
                    record.TotalMeters = TotalLength;
                    // Debug.WriteLine(opration);
                ...
                {
                    Color = (string)(MySheet.Cells[index, 4].Value.ToString());
                    record.Color = Color;
